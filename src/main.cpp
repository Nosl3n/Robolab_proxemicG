...
/** \mainpage RoboComp::gridder
 *
 * \section intro_sec Introduction
 *
 * The gridder component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/gridder --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class gridder : public RoboComp::Application
{
public:
	gridder (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::gridder::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		{
			// Server adapter creation and publication
			if (not GenericMonitor::configGetString(communicator(), prefix, "Gridder.Endpoints", tmp, ""))
			{
				cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy Gridder";
			}
			Ice::ObjectAdapterPtr adapterGridder = communicator()->createObjectAdapterWithEndpoints("Gridder", tmp);
			auto gridder = std::make_shared<GridderI>(worker);
			adapterGridder->add(gridder, Ice::stringToIdentity("gridder"));
			adapterGridder->activate();
			cout << "[" << PROGRAM_NAME << "]: Gridder adapter created in port " << tmp << endl;
		}
		catch (const IceStorm::TopicExists&){
			cout << "[" << PROGRAM_NAME << "]: ERROR creating or activating adapter for Gridder\n";
		}
...
int main(int argc, char* argv[])
{
	...
	::gridder app(prefix, startup_check_flag);
